...
#define CRYPT_FS_ROOT_DIR_BLOCK                                                \
    (sizeof(struct CryptFS_Header) - sizeof(struct CryptFS_Directory))
...
struct CryptFS_Header
{
    ...
    struct Crypt_FS_Key
        keys[NB_ENCRYPTION_KEYS]; // Encrypted keys of users (max
                                  // NB_ENCRYPTION_KEYS users)
